...
import java.util.ArrayList;
	...
	public class DataSet {
		 ...
		 */
		protected int numIns, numOuts, numItems, tWidth;
		protected String inFormat, outFormat, rawFormat;
		protected ArrayList<String>inNames = new ArrayList<String>();
		protected ArrayList<String>outNames = new ArrayList<String>();
		protected ArrayList<DataItem> allItems = new ArrayList<DataItem>();
		private ArrayList<ArrayList<Double>> sseLog = new ArrayList<ArrayList<Double>>();
		...
		protected class DataItem {
			private ArrayList<Double> dataInputs = new ArrayList<Double>();
			private ArrayList<Double> dataOutputs = new ArrayList<Double>();
			private ArrayList<Double> dataTargets = new ArrayList<Double>();
			...
			public DataItem(int numIns, String data) {
				...
				for (int ct=0; ct<dataSplit.length; ct++) {		// for each
					...
					if (ct<numIns) dataInputs.add(d);			// if is input, add to dataInputs
						...
						dataOutputs.add(0.0);					// add space for dataOutputs
					}	
				}
			}
			/**
			 ...
			 */
			public void storeOutput(int pos, double d) {
			 ...
			 * @param d		value of output
			 */
			public void storeOutputs(ArrayList<Double> d) {
		...
		public DataSet(String alldata) {
			...
			numIns = Integer.parseInt(fLine[0]);					
			numOuts = Integer.parseInt(fLine[1]);
			if (fLine.length>2) inFormat = fLine[2]; else inFormat = "%.1f";
			if (fLine.length>3) outFormat = fLine[3]; else outFormat = "%.1f";
			if (fLine.length>4) rawFormat = fLine[4]; else rawFormat = "%.2f";
			
			setNames(allSplit[1].split(" "));						// process second line
		 ...
		 * @param d
		 */
		public void storeOutput(int item, int pos, double d) {
			allItems.get(item).storeOutput(pos, d);
		}
		/**
		 ...
		 */
		public void storeOutputs(int item, ArrayList<Double>d) {
			allItems.get(item).storeOutputs(d);
		}
...
//				s = s + String.format(fStr, vals.get(ct)) + "\t";
		...
		public String dataAnalysis() {
			...
			String s = "SSE ";
			for (int ct=0; ct<numOuts; ct++) s = s + String.format("%.3f", SSE.get(ct)) + " ";
